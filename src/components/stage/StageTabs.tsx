...
const StageTabs = ({ onOpen, variant, active }: Props) => {
  ...
  return (
    <div className={`bg-background ${variant === 'mobile' ? 'px-4 py-6' : 'mx-auto max-w-[1400px] px-5 pb-6 pt-5 md:px-10'}`}>
      <h3 className="text-center font-display text-[1.15rem] leading-tight text-foreground md:text-[1.45rem]">
        Сделай сейчас по выбранному этапу
      </h3>

      <div className={`mt-4 grid gap-3 ${variant === 'mobile' ? 'grid-cols-1' : 'grid-cols-2 lg:grid-cols-4'}`}>
        {TAB_ROWS.map((r) => {
          ...
          return (
            <button
              ...
              type="button"
              onClick={() => onOpen(r.id)}
              className={`flex min-h-[86px] flex-col items-center justify-center gap-1.5 rounded-2xl border px-4 py-4 text-center transition-colors ${
                on
                  ? 'border-primary bg-primary/5 text-foreground'
                  : 'border-border bg-card text-foreground hover:border-primary/50'
              }`}
            >
              <span className="text-[0.88rem] font-medium leading-tight">{r.label}</span>
      ...
      <p className="mx-auto mt-4 max-w-[48em] text-center text-[0.8rem] leading-relaxed text-muted-foreground">
