...
def handler(event: dict, context) -> dict:
    ...
    if not key:
        return {
            ...
            'headers': CORS,
            'body': json.dumps({
                'answer': 'Проверка по нормам ещё не активирована: не добавлен ключ доступа к языковой модели. '
                          'Добавьте секрет GROQ_API_KEY в настройках проекта — и сервис заработает.',
                'configured': False,
            }, ensure_ascii=False),
        }
